...
type Plan = { product: AldiProduct; disposition: ImportDisposition; reason: string; productId: string | null; storeProductId: string | null };
...
function categoryForAldi(product: AldiProduct) {
  ...
  if (/(?:detergent|dishwashing|toilet|tissue|cleaner|bleach)/.test(name)) return { category: "Household", productType: ProductType.HOUSEHOLD };
...
async function plansForPage(products: AldiProduct[], aliasesSeen: Set<string>) {
  ...
  return products.map<Plan>((product) => {
    ...
    const listing = listingById.get(product.externalId); if (listing) return { product, disposition: "retain", reason: "authoritative ALDI listing already exists", productId: listing.productId, storeProductId: listing.id };
    ...
    return { product, disposition: "create", reason: "unique public ALDI catalogue identity; queued for later barcode verification", productId: randomUUID(), storeProductId: randomUUID() };
  });
}
...
async function attach(plans: Plan[]) {
  ...
  await prisma.$transaction(async (tx) => {
    if (creates.length) {
      await tx.product.createMany({ data: creates.map((plan) => { const mapped = categoryForAldi(plan.product); return { id: plan.productId!, name: plan.product.name, canonicalName: plan.product.name, slug: `${slugifyProductName(plan.product.name)}-aldi-${plan.product.externalId}`, brand: plan.product.brand, category: mapped.category, packSize: plan.product.packSize, imageUrl: plan.product.imageUrl, productType: mapped.productType, lifecycle: ProductLifecycle.REVIEW_REQUIRED, confidenceScore: 0.65 }; }) });
