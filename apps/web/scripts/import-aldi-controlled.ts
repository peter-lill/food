...
async function plansForPage(products: AldiProduct[], aliasesSeen: Set<string>) {
  ...
  return products.map<Plan>((product) => {
    ...
    return { product, disposition: "create", reason: "unique public ALDI catalogue identity; queued for later barcode verification", productId: randomUUID(), storeProductId: randomUUID(), category: categoryResolutionForImport(product.name, comparableCategories) };
  });
}
