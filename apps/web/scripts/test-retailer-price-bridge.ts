...
assert.match(composeSource, /\$\{GROCERY_MCP_HOST_PORT:-8790\}:8790/, "Food publishes its grocery bridge on port 8790");
...
assert.match(groceryDockerfile, /looseversion==1\.3\.0/, "Python 3.12 installs the legacy version API expected by the pinned UC patcher");
assert.match(colesBrowserSource, /configure_uc_version_parser\(uc_patcher, LooseVersion\)[\s\S]*uc\.Chrome\(/, "the compatible version parser must be installed before UC patches ChromeDriver");
