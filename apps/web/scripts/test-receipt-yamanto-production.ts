...
const productionBlockFragments = [
...
const preferredLines = receiptLinesFromBlocks(
  [{ paragraphs: [{ lines: productionBlockFragments }] }],
  expectedRawText,
);
...
RAY or 16 00 ong 0.00
`;
const repairedLines = receiptLinesFromBlocks(undefined, damagedProductionText);
const repairedRoles = classifyReceiptLines(repairedLines);
assert.equal(repairedLines.some((line) => line.text === "TOTAL $35.60"), true, "split tender payments should recover the receipt total");
assert.equal(repairedLines.some((line) => /Lhe AL|cme A|INCI UDED|RAY or/i.test(line.text)), false, "payment, tax and footer fragments must be removed before parsing");
...
assert.equal(repairedRoles.some((line) => line.role === "tax" || line.role === "tender" || line.role === "footer"), false);
