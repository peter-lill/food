...
function filterSql(filter: ReviewFilter) {
  switch (filter) {
    case "review": return Prisma.sql`AND (p."lifecycle" = 'REVIEW_REQUIRED' OR r."status" = 'REVIEW_REQUIRED' OR cardinality(COALESCE(q."issues", ARRAY[]::text[])) > 0)`;
    case "queued": return Prisma.sql`AND r."status" IN ('QUEUED', 'RUNNING')`;
    case "failed": return Prisma.sql`AND r."status" = 'FAILED'`;
...
export async function getCatalogueReviewQueue(filter: ReviewFilter = "all", limit = 200): Promise<CatalogueReviewItem[]> {
  const rows = await prisma.$queryRaw<Array<{
    ...
    issues: string[] | null;
    repairReasons: string[] | null;
    repairStatuses: string[] | null;
    ...
    SELECT
      ...
      COALESCE(array_agg(DISTINCT r."reason") FILTER (WHERE r."reason" IS NOT NULL), ARRAY[]::text[]) AS "repairReasons",
      COALESCE(array_agg(DISTINCT r."status"::text) FILTER (WHERE r."status" IS NOT NULL), ARRAY[]::text[]) AS "repairStatuses",
      MIN(r."priority") AS "priority",
      MAX(r."lastError") AS "lastError",
      g."status"::text AS "gtinStatus",
      GREATEST(p."updatedAt", COALESCE(MAX(r."updatedAt"), p."updatedAt"), COALESCE(q."calculatedAt", p."updatedAt")) AS "updatedAt"
    FROM "Product" p
    LEFT JOIN "ProductQualitySnapshot" q ON q."productId" = p."id"
    LEFT JOIN "ProductRepairItem" r ON r."productId" = p."id" AND r."status" IN ('QUEUED', 'RUNNING', 'REVIEW_REQUIRED', 'FAILED')
    LEFT JOIN "ProductGtinIdentity" g ON g."productId" = p."id"
    WHERE p."lifecycle" <> 'ARCHIVED'
      AND (q."overallScore" < 100 OR cardinality(COALESCE(q."issues", ARRAY[]::text[])) > 0 OR r."id" IS NOT NULL OR p."lifecycle" = 'REVIEW_REQUIRED')
      ${filterSql(filter)}
    GROUP BY p."id", q."productId", g."status"
    ORDER BY
      CASE WHEN p."lifecycle" = 'REVIEW_REQUIRED' OR g."status" = 'CONFLICT' THEN 0 ELSE 1 END,
      COALESCE(MIN(r."priority"), 999),
