...
export function guardedProductIdentity(value: string): ProductClassification | null {
  ...
  if (has(text, ["baby mum mum", "little bellies", "little quacker"]) || /\b(?:4|6|8|10|12)\+?\s*months?\b/.test(text) || /\b1\s*-\s*4\s*years?\b/.test(text)) {
    if (has(text, ["rusk", "rusks", "puff", "puffs", "snack", "bar", "food", "puree", "custard", "meal", "meals", "cereal", "pasta bake"])) return result("Baby", "Baby food & care", "baby age/stage product identity");
  }

  // Shelf-stable/canned seafood must precede culinary oils. "In vegetable oil"
  // describes the packing medium, not the product identity.
  if (has(text, ["tuna", "sardine", "sardines", "mackerel", "anchovy", "anchovies"]) && (has(text, ["chunks", "slices", "fillets in oil", "in oil", "in vegetable oil", "in olive oil", "in sauce", "tomato sauce"]) || /\b(?:50|90|95|105|110|125|185|400|415|425)g\b/.test(text)))
    return result("Pantry", "Canned food, soups & noodles", "shelf-stable seafood identity");

  // Pickled/antipasto identities before bakery/dairy ingredient words.
  ...
  // Finished bakery goods before cinnamon/herb/grain modifiers; flour and mixes
  // remain pantry baking ingredients.
  if (has(text, ["muffins", "muffin 4 pack", "bread", "breadsticks", "bread sticks", "croissant", "brioche", "bagel", "bakery loaf"]) && !has(text, ["bread mix", "bread crumbs", "breadcrumbs", "bread flour", "pizza flour", "plain flour", "muffin mix"]))
  ...
  // Baking ingredients/mixes before bakery product words.
  ...
  // Biscuits/crackers/snacks before dairy or flavour words.
  ...
  // Birds Eye potato/fish lines are freezer identities. Keep this before generic
  // chips/snack matching so lattice/sidewinders do not fall into Pantry.
  ...
  if (has(text, ["rice bran oil", "olive oil", "extra virgin olive oil", "canola oil", "vegetable oil", "sunflower oil", "sesame oil"]) && !has(text, ["breadstick", "breadsticks", "cracker", "crackers", "spread", "sardine", "sardines", "tuna", "anchovy", "anchovies", "mackerel"]))
