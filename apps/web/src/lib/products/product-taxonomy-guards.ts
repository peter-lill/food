...
export function guardedProductIdentity(value: string): ProductClassification | null {
  ...
  if (has(text, ["follow on formula", "newborn formula", "infant formula", "stage 1 formula", "stage 2 formula"])) return result("Baby", "Baby food & care", "infant formula identity");
  if (has(text, ["digital kitchen timer", "mechanical kitchen scale"]) || (has(text, ["appetito"]) && has(text, ["digital timer", "60 minute timer", "high temp baster", "smoothie straws"]))) return result("Home, kitchen & appliances", "Kitchen tools & utensils", "kitchen tool identity");
  if (has(text, ["appetito mesh produce bags", "mesh produce bags"])) return result("Household", "Food storage & household", "reusable food storage identity");
  if (has(text, ["windscreen wash", "windshield wash"])) return result("Automotive", "Car care", "automotive product identity");
  if (has(text, ["armor all"]) && has(text, ["leather wipes", "microfibre", "microfiber", "protectant", "glass wipes", "synthetic chamois"])) return result("Automotive", "Car care", "automotive product identity");
  ...
  if (has(text, ["razor system", "denture cleaning tablets", "denture cleanser tablets", "ibuprofen tablets", "ibuprofen liquid capsules", "ibuprofen capsules", "lozenges"])) return result("Health & personal care", "Health & personal care", "personal-care product identity");
  ...
  if ((has(text, ["command"]) && has(text, ["picture strip", "picture strips", "utility adhesive hook", "utility adhesive hooks", "mounting hook", "mounting hooks", "hanging hook", "hanging hooks", "utensil hook", "utensil hooks"])) || has(text, ["command adhesive picture hanging", "command clear mini hanging hooks", "command clear utensil hooks", "motion sensor automatic bin", "automatic bin"])) return result("Household", "Home organisation & storage", "home organisation product identity");
  ...
  if (snackNGo || has(text, ["breakfast biscuits", "custard creme biscuits", "custard crème biscuits", "arnotts shapes", "arnott's shapes", "vita weat", "cracker", "crackers", "cracker chips", "crispbread", "potato chips", "vege chips", "vege crisps", "lentil chips", "lentil crisps", "chickpea chips", "hummus chips", "corn chips", "rice crackers", "pea crisps", "pork crackle", "mixed nuts", "cereal bar", "cereal bars", "fruit filled bar", "fruit filled bars", "filled bars", "protein bar", "protein bars", "paleo bar", "paleo bars", "nut bar", "nut bars", "tortilla strips"])) return result("Pantry", "Snacks", "packaged snack identity");
  if (has(text, ["muffin mix", "cake mix", "brownie mix", "cookie mix", "bread mix", "custard powder", "bread crumbs", "breadcrumbs", "panko bread crumbs", "bread improver", "cream of tartar", "egg replacer", "bread and pizza plain flour", "bread & pizza plain flour", "almond meal"])) return result("Pantry", "Baking", "baking mix or ingredient identity");
  if (has(text, ["muffins", "muffin 4 pack", "cinnamon scroll", "cinnamon scrolls", "bread", "breadsticks", "bread sticks", "bruschetta toasts", "toasts", "croissant", "brioche", "bagel", "bakery loaf", "sourdough vienna"]) && !has(text, ["bread mix", "bread crumbs", "breadcrumbs", "bread flour", "pizza flour", "plain flour", "muffin mix", "improver"])) return result("Bakery", "Bread & bakery", "bakery product identity");
  if (has(text, ["chicken drumstick", "chicken drumsticks", "chicken wing", "chicken wings", "chicken breast", "chicken thigh", "chicken thighs", "chicken bites", "chicken burger", "chicken burgers", "pork shoulder", "pork loin", "pork ribs", "beef brisket", "beef roast", "beef soup bones", "beef sausage", "beef sausages", "honey beef sausages", "lamb leg", "lamb shank", "lamb shanks", "salmon fillet", "salmon fillets", "hoki fillet", "fish fillet", "prawn skewers", "prawn cutlets", "beef mince", "beef jerky", "meatballs", "air dried venison", "biltong", "salt and pepper squid", "salt & pepper squid"]) && !has(text, ["recipe base", "meatball mould", "meatball mold"])) return result("Meat & seafood", "Fresh meat & seafood", "meat or seafood product identity");
