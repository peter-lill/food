...
export function guardedProductIdentity(value: string): ProductClassification | null {
  ...
  if (has(text, ["leg ham", "virginian ham", "mortadella", "prosciutto", "salami", "charcuterie", "pate"])) return result("Deli", "Deli meat & antipasto", "deli product identity");
  ...
  if (has(text, ["rice pudding", "dessert sauce", "flavoured topping", "fudge topping", "strawberry topping", "maple flavoured syrup", "waffle cones", "waffle cone"])) return result("Pantry", "Desserts", "dessert product identity");
