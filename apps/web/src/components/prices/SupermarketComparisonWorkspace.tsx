...
type ProductComparison = { key: string; name: string; retailerPrices: Array<{ retailer: SupermarketRetailer; price: SupermarketPriceView }>; bestPrice: SupermarketPriceView; savings: number };
type ItemRetailerMatch = { retailer: SupermarketRetailer; price: SupermarketPriceView; estimate: number };
type ShoppingItemComparison = { item: SupermarketShoppingItem; matches: ItemRetailerMatch[]; best: ItemRetailerMatch | null };
...
function normaliseName(value: string) { return value.toLocaleLowerCase("en-AU").replace(/[^a-z0-9]+/g, " ").trim(); }
function nameTokens(value: string) {
  return new Set(normaliseName(value).split(" ").filter((token) => token.length > 1).map((token) => token.endsWith("s") && token.length > 3 ? token.slice(0, -1) : token));
}
function matchScore(itemName: string, productName: string) {
  const item = normaliseName(itemName); const product = normaliseName(productName);
  if (!item || !product) return 0;
  if (item === product) return 100;
  if (item.includes(product) || product.includes(item)) return 85;
  const itemSet = nameTokens(item); const productSet = nameTokens(product);
  const overlap = [...itemSet].filter((token) => productSet.has(token)).length;
  const ratio = overlap / Math.max(itemSet.size, productSet.size, 1);
  return ratio >= 0.5 ? Math.round(45 + ratio * 35) : 0;
}
function comparisonValue(price: SupermarketPriceView) { return price.unitPrice ?? price.price; }
...
function compareShoppingItem(item: SupermarketShoppingItem, prices: SupermarketPriceView[], retailers: readonly SupermarketRetailer[]): ShoppingItemComparison {
  ...
  }).filter((match): match is ItemRetailerMatch => match !== null).sort((left, right) => left.estimate - right.estimate);
  ...
  return (
    <details className={`card ${styles.capture}`}>
      ...
      <form action={action} className={styles.captureForm}>
        ...
        <div className={styles.captureActions}><CaptureSubmitButton /><span>New entries become the latest price for the matching retailer, product, brand and pack size.</span></div>
      </form>
    </details>
  );
}
...
function ProductMode({ comparisons, retailerCount }: { comparisons: ProductComparison[]; retailerCount: number }) {
  ...
  return (
    <section className={styles.modePanel}>
      <div className={styles.modeHeading}><div><p className="eyebrow">INDIVIDUAL ITEMS</p><h2>Compare a product</h2><p className="subtle">The cheapest comparable unit price is used when one is available; otherwise shelf price is used.</p></div><label className="field"><span>Search products</span><input onChange={(event) => setQuery(event.target.value)} placeholder="e.g. milk or salmon" type="search" value={query} /></label></div>
      {comparisons.length === 0 ? <div className="pantry-empty"><strong>No supermarket prices yet.</strong><p>Add catalogue or shelf prices below to begin comparing stores.</p></div>
        ...
        <article className={styles.productCard} key={comparison.key}>
          ...
          <div className={styles.storeRows}>{comparison.retailerPrices.map(({ retailer, price }, index) => (
            <div className={index === 0 ? styles.bestStoreRow : styles.storeRow} key={`${comparison.key}-${retailer}`}>
              ...
              <div>{price.isSpecial ? <small>Special</small> : null}<strong>{money(price.price)}</strong><span>{price.unitPrice ? `${money(price.unitPrice)} unit price` : "Shelf price comparison"}</span></div>
            </div>
          ))}</div>
...
function ShoppingListMode({ data }: { data: SupermarketComparisonData }) {
  ...
  const bestCompleteStore = storeTotals.filter((store) => store.missingCount === 0).sort((left, right) => left.total - right.total)[0] ?? null;
  const splitSavings = bestCompleteStore ? roundMoney(bestCompleteStore.total - splitTotal) : null;
  if (data.shoppingLists.length === 0) return <section className={styles.modePanel}><div className="pantry-empty"><strong>No Shopping lists available.</strong><p>Create a Shopping list first, then return here to estimate retailer totals.</p></div></section>;
  return (
    <section className={styles.modePanel}>
      <div className={styles.modeHeading}><div><p className="eyebrow">WHOLE LIST</p><h2>Compare a shopping list</h2><p className="subtle">Search current online prices or use Food&apos;s saved catalogue observations for an estimate.</p></div><label className="field"><span>Shopping list</span><select onChange={(event) => setSelectedListId(event.target.value)} value={selectedList?.id ?? ""}>{data.shoppingLists.map((list) => <option key={list.id} value={list.id}>{list.name} · {list.items.length} remaining</option>)}</select></label></div>
      {selectedList && selectedList.items.length === 0 ? <div className="pantry-empty"><strong>This Shopping list has no remaining items.</strong><p>Add or uncheck items before comparing retailers.</p></div>
      : selectedList ? <>
        <LiveShoppingPriceSearch key={selectedList.id} list={selectedList} />
        <div className={styles.savedEstimateHeading}><div><p className="eyebrow">SAVED PRICE ESTIMATE</p><h3>Food catalogue prices</h3><p className="subtle">These totals use prices previously imported, captured or cached from an earlier live search.</p></div></div>
        <div className={styles.totalGrid}>{storeTotals.map((store) => (
          <article className="card" key={store.retailer}><p className="eyebrow"><RetailerLogo compact retailer={store.retailer} /></p><strong className={styles.totalValue}>{store.matchedCount ? money(store.total) : "—"}</strong><p>{store.matchedCount}/{itemComparisons.length} items matched{store.missingCount ? ` · ${store.missingCount} missing` : " · complete"}</p></article>
        ))}<article className={`card ${styles.splitCard}`}><p className="eyebrow">SPLIT SHOP</p><strong className={styles.totalValue}>{splitMatchedCount ? money(splitTotal) : "—"}</strong><p>{splitMatchedCount}/{itemComparisons.length} items matched{splitSavings !== null && splitSavings > 0 ? ` · saves ${money(splitSavings)}` : " · cheapest item-by-item"}</p></article></div>
        <div className={styles.listRows}>{itemComparisons.map((comparison) => (
          <article className={styles.listRow} key={comparison.item.id}><div className={styles.listItemName}><strong>{comparison.item.name}</strong><span>{comparison.item.quantity ?? 1} {comparison.item.unit ?? "item"}</span></div>
          {comparison.matches.length === 0 ? <span className="badge neutral">No price match</span> : <div className={styles.matchPills}>{comparison.matches.map((match, index) => <span className={index === 0 ? styles.bestMatch : styles.match} key={`${comparison.item.id}-${match.retailer}`}><small><RetailerLogo compact retailer={match.retailer} /></small><strong>{money(match.estimate)}</strong></span>)}</div>}</article>
        ))}</div>
        <p className={styles.estimateNote}>{bestCompleteStore ? <>Best single-store estimate: <RetailerLogo compact retailer={bestCompleteStore.retailer} /> at {money(bestCompleteStore.total)}.</> : "No retailer currently has a saved price match for every remaining item."} Product matching is automatic and should be checked before relying on the estimate.</p>
      </> : null}
    </section>
  );
}
