...
import { environmentProvider } from "@/lib/ai/provider-settings";
...
async function saveSettings(formData: FormData) {
  ...
  if (!apiKey && !existing) redirect(`/admin/ai-settings?provider=${provider}&result=${environmentProvider(provider) ? "environment-managed" : "key-required"}`);
...
async function testSettings(formData: FormData) {
  ...
  const environment = environmentProvider(provider);
  if (!setting && !environment) redirect(`/admin/ai-settings?provider=${provider}&result=key-required`);
  let succeeded = false;
  try {
    const configuration = setting
      ? { apiKey: decryptProviderSecret(setting), baseUrl: setting.baseUrl, model: setting.model }
      : environment!;
    if (provider === "aicompute") await testAiComputeConnection(configuration);
    else {
      const response = await fetch(`${configuration.baseUrl.replace(/\/$/, "")}/models`, { headers: { Authorization: `Bearer ${configuration.apiKey}` }, signal: AbortSignal.timeout(30_000) });
      if (!response.ok) throw new Error(`OpenAI returned ${response.status}.`);
    }
    if (setting) await prisma.aiProviderSetting.update({ where: { provider }, data: { lastTestedAt: new Date(), lastTestOk: true, lastTestMessage: "Connection successful" } });
    succeeded = true;
  } catch (error) {
    const message = error instanceof Error ? error.message.slice(0, 180) : "Connection failed";
    if (setting) await prisma.aiProviderSetting.update({ where: { provider }, data: { lastTestedAt: new Date(), lastTestOk: false, lastTestMessage: message } });
  }
...
export default async function AiSettingsPage({ searchParams }: { searchParams: Promise<{ result?: string; provider?: string }> }) {
  ...
  const notice = result === "saved" ? "Settings saved. Test the connection before this provider is enabled."
    ...
    : result === "environment-managed" ? "This key is managed by the production environment. Enter a new key only if you want Admin to replace it."
  ...
  return <main className={styles.page}>
    ...
    {Object.entries(providers).map(([providerKey, definition]) => {
      ...
      const environment = environmentProvider(key);
      const testedNow = active === key && result === "test-ok";
      const failedNow = active === key && result === "test-failed";
      return <section className={styles.panel} key={key}>
        <div><p className="eyebrow">{definition.purpose}</p><h2>{definition.title}</h2></div>
        <dl className={styles.status}><div><dt>API key</dt><dd>{setting ? "Configured in Admin ••••••••" : environment ? "Configured in deployment environment ••••••••" : "Not configured"}</dd></div><div><dt>Connection</dt><dd>{testedNow || setting?.lastTestOk === true ? "Tested and ready" : failedNow || setting?.lastTestOk === false ? "Test failed" : "Not tested"}</dd></div></dl>
        <form action={saveSettings} className={styles.form}>
          <input name="provider" type="hidden" value={key} />
          <label><span>API key</span><input autoComplete="off" name="apiKey" placeholder={setting ? "Leave blank to keep current key" : environment ? "Enter only to replace environment key" : "Paste API key"} type="password" /></label>
        ...
        {setting || environment ? <form action={testSettings}><input name="provider" type="hidden" value={key} /><button className="secondary-button" type="submit">Test connection</button></form> : null}
      </section>;
    })}
