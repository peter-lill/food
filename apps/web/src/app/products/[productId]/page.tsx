...
function pantryQuantityLabel(inventory: Array<{ quantity: number; unit: string }>) {
  ...
  return [...totals.entries()].map(([unit, quantity]) => `${formatQuantity(quantity)} ${unit === "item" && quantity !== 1 ? "items" : unit}`).join(" + ");
}
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const retailerCount = new Set([...product.storeProducts.map((listing) => listing.retailer), ...product.priceObservations.map((observation) => observation.retailer)]).size;
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.detailHero}>
        <div className={styles.identity}><div className={styles.identityLayout}>
          <div className={styles.productVisual}><img alt={displayName} src={productImage} /></div>
          <div>
            <p className="eyebrow">{department ?? "PRODUCT"}</p><h1 className="page-title">{displayName}</h1>
            ...
            <div className={styles.identityMeta}>{product.packSize ? <span>{product.packSize}</span> : null}{product.barcode ? <span>Barcode {product.barcode}</span> : barcodeRequired ? <span>Barcode not known</span> : null}{product.recipes.length ? <span>{product.recipes.length} recipe link{product.recipes.length === 1 ? "" : "s"}</span> : null}{retailerCount ? <span>{retailerCount} retailer{retailerCount === 1 ? "" : "s"}</span> : null}</div>
          </div>
        </div></div>
        <aside className={styles.panel}><p className="eyebrow">YOUR PRODUCT</p><div className={styles.metricGrid}>
      ...
      <section className={styles.sections}>
        {product.barcode ? <article className={`${styles.panel} ${styles.barcodePanel}`}><div><p className="eyebrow">GTIN / EAN</p><h2>Product barcode</h2><p className="subtle">Use this barcode to identify the exact packaged product across retailers and product databases.</p></div><div className={styles.barcodeGraphic}><Ean13Barcode value={product.barcode} /></div></article> : barcodeRequired ? <article className={`${styles.panel} ${styles.barcodePanel}`}><div><p className="eyebrow">GTIN / EAN</p><h2>Barcode not known</h2><p className="subtle">Food will continue checking trusted retailer and barcode sources for this product.</p></div></article> : null}

        <article className={styles.panel}><details><summary><strong>Edit product details</strong></summary><form action={editAction} className="pantry-form compact">
        ...
        <article className={styles.panel}><h2>Product identity</h2><ul className={styles.list}>
